...
object EntitiesUpdate {
  ...
  def main(args: Array[String]) {
    ...
    var entities =  datasets.head.task.cache.entities
    entities = datasets.head.task.propertyClusters.get.updateEntitiesWithSchemaInfo(entities)
   
  
    entities.positive.values.filter(_.source.uri.equals("http://dws.products.wdc/node7e152b885767185b076f741cc97ea5"))
    .foreach(p => p.source.updateAfterReclustering(13, 1, 72))
  }
}
